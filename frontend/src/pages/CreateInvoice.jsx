...
const res = await fetch(
  `${API_BASE}/api/invoices?invoiceNumber=${encodeURIComponent(candidate)}`,
  { method: "GET", headers }
);
        if (!res.ok) {
  ...
  const fetchBusinessProfile = async () => {
    try {
      ...
      if (!res.ok) {
        console.warn("Business profile request failed:", res.status);
        return;
      }
      ...
      setInvoice((prev) => {
        ...
        return {
          ...
          fromBusinessName:
            prev.fromBusinessName || serverProfile.businessName,

          fromEmail:
            prev.fromEmail || serverProfile.email,

          fromAddress:
            prev.fromAddress || serverProfile.address,

          fromPhone:
            prev.fromPhone || serverProfile.phone,

          fromGst:
            prev.fromGst || serverProfile.gst,
