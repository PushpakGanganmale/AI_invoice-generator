...
import AiInvoiceModal from "../component/AiInvoiceModel";
import GeminiIcon from "../component/GeminiIcon";
import { useNavigate } from "react-router-dom";
import { invoicesStyles } from "../assets/dummyStyles";
import { useAuth } from "@clerk/clerk-react";
const API_BASE = import.meta.env.VITE_API_URL || "https://ai-invoice-generator-backend-91pw.onrender.com";
...
export default function InvoicesPage() {
 ...
 async function handleGenerateFromAI(rawText) {
  try {
    ...
    const aiRes = await fetch(`${API_BASE}/api/ai-invoices/generate`, {
      method: "POST",
      headers: {
    ...
    if (!aiRes.ok) {
      ...
      if (
        ...
        throw new Error(`AI quota exceeded: ${serverMessage}`);
      }
    ...
    if (!aiJson) {
      try {
        aiJson = JSON.parse(bodyText || "");
      } catch {
        aiJson = null;
      }
    }

    const aiRaw = aiJson?.data || aiJson;

    if (!aiRaw) {
      throw new Error("AI returned no invoice data");
    }

    const client = normalizeClient(aiRaw.client);

    const items = Array.isArray(aiRaw.items)
      ? aiRaw.items.map((it) => ({
  ...
  items,
  currency: "INR",
  taxPercent: Number(aiRaw.taxPercent ?? 0),
  status: "draft",
};
    ...
    if (!tokenForCreate) {
      throw new Error(
        "Creating invoice requires sign-in. Please sign in first."
      );
    }
    ...
    alert(err.message || "Failed to generate invoice with AI");
  }
}
